...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate authentication - replace with Firebase/Auth.js
      if (email && password) {
        localStorage.setItem('user', JSON.stringify({ email, isAdmin: email.includes('admin') }));
        router.push('/menu');
      } else {
      ...
      >
        ...
        >
          ...
          <motion.button
            ...
            className="w-full border border-border py-2 rounded-md font-medium text-foreground hover:bg-muted transition-colors flex items-center justify-center gap-2"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
